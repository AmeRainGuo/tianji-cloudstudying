...
public interface PointsRecordMapper extends BaseMapper<PointsRecord> {
    ...
    Integer queryUserPointsByTypeAndDate(@Param(Constants.WRAPPER) QueryWrapper<PointsRecord> queryWrapper);

    @Select("select type ,sum(points) as points from points_record ${ew.customSqlSegment} group by type")
    List<PointsRecord> queryUserPointsByDate(QueryWrapper<PointsRecord> wrapper);
}
